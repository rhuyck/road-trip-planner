...
export function Sidebar({ collapsed, onToggleCollapsed, onAddStop, onEditStop, onEditHotel, onOpenLists, isGuest }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    // Gas estimate: $6.00/gal in CA, $4.50/gal elsewhere, at 25 mpg
    let gasTotal = 0;
    for (const d of days) {
      const r = routes[d.id];
      if (!r) continue;
      const miles = r.distanceMeters / 1609.34;
      const price = d.state === 'CA' ? 6.00 : 4.50;
    ...
    <div className="flex flex-col h-full bg-stone-50 dark:bg-gray-900 border-r border-stone-200 dark:border-gray-700 w-96 flex-shrink-0">
      <div className="p-4 border-b border-stone-200 dark:border-gray-700">
        ...
        <div className="grid grid-cols-2 gap-2 mt-3">
          ...
          <Stat label="Gas Est." value={stats.gasTotal > 0 ? `~$${Math.round(stats.gasTotal).toLocaleString()}` : '—'} sub="$4.50–$6.00/gal · 25mpg" />
        </div>
      </div>
