...
export function AddStopModal({
  ...
  const field = (key: keyof typeof form, label: string, placeholder = '', opts?: { optional?: boolean; textarea?: boolean }) => (
    <div>
      ...
      {opts?.textarea ? (
        <textarea
          className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-400 resize-none"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-gray-800 border border-gray-600 rounded-2xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-gray-700">
          <div>
            <h2 className="text-lg font-semibold text-white">{editingStop ? 'Edit Stop' : 'Add Stop'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                type="text"
                className="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-400"
          ...
          {showLegs && (
            <div className="bg-gray-700/60 rounded-xl p-3 space-y-2">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Drive distances</p>
              {computingLegs && !legToStop && !legFromStop && (
                <p className="text-xs text-gray-500 animate-pulse">Computing…</p>
              )}
              {legToStop && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-400 truncate mr-2">From {dayOriginCity}</span>
              ...
              {legFromStop && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-400 truncate mr-2">To {dayDestCity}</span>
                  <span className="text-white font-medium whitespace-nowrap">
              ...
              {legToStop && legFromStop && (
                <div className="flex items-center justify-between text-sm border-t border-gray-600 pt-2">
                  <span className="text-gray-400">Total leg</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  className="w-full bg-gray-700 border border-gray-600 rounded-lg pl-7 pr-3 py-2 text-white text-sm focus:outline-none focus:border-blue-400"
